...
let UpdateProductSimple = function (
  ncUtil,
  channelProfile,
  flowContext,
  payload,
  ...
  //If a productDocument or productRemoteID was not passed in, the request is invalid
  if (!payload) {
    invalid = true;
  ...
  if (!invalid) {
    ...
    let readUrl = url + "/" + payload.productRemoteID + minorVersion;
        ...
        if (!error) {
          if (response.statusCode === 200) {
            ...
            */
            ...
            // Add productRemoteID to Id of doc as it's required for this function
            payload.doc.Item.Id = payload.productRemoteID;
            ...
            request(options, function (error, response, body) {
              try {
                if (!error) {
                  ...
                  logError("Do UpdateProduct Callback error - " + error);
                  out.payload.error = {err: error};
                  out.ncStatusCode = 400;
                  callback(out);
                }
              } catch (err) {
                logError("Exception occurred in UpdateProduct - " + err);
                out.ncStatusCode = 500;
